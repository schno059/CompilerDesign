...
class ASTExprMethodCall extends ASTExpr {
  ...
  public ASTExpr exprValue() {
    ...
    {
      STVarDeclEntry variableEntry = (STVarDeclEntry) methodSTE.methodScope.entries.get(i);
      variableEntry.exprValue = arguments.get(i).exprValue();
    }
    for (ASTStmt stmt : methodSTE.stmts) {
      stmt.run();
      if(stmt.isReturn()) return stmt.returnExpr();
    }
    return this;
  }
